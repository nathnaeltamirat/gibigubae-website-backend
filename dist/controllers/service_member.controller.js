...
export const getMemberById = async (req, res, next) => {
    try {
        ...
        if (!member) {
            const error = new Error("Service member not found");
...
export const createMember = async (req, res, next) => {
    try {
        ...
        if (!existingGroup) {
            const error = new Error("Service group not found");
            error.statusCode = 404;
            throw error;
        }
        let subGroupEntity = null;
        if (sub_group_id) {
            subGroupEntity = await subGroupRepo.findOneBy({ id: sub_group_id });
            if (!subGroupEntity) {
                const error = new Error("Service sub-group not found");
                error.statusCode = 404;
                throw error;
            }
        }
...
export const deleteMember = async (req, res, next) => {
    try {
        ...
        if (user.role !== "super_admin") {
            const error = new Error("Forbidden: Only Super Admin");
            error.statusCode = 403;
            throw error;
        }
        const { user_id, group_id } = req.params;
...
//# sourceMappingURL=service_member.controller.js.map