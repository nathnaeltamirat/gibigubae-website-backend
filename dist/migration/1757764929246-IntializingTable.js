export class IntializingTable1757764929246 {
    name = 'IntializingTable1757764929246';
    async up(queryRunner) {
        await queryRunner.query(`CREATE TABLE "confession_father" ("id" SERIAL NOT NULL, "full_name" character varying NOT NULL, "phone_number" character varying NOT NULL, CONSTRAINT "PK_d962ec61152f15e3935d98980f9" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "student" ("id" SERIAL NOT NULL, "first_name" character varying NOT NULL, "father_name" character varying NOT NULL, "grand_father_name" character varying NOT NULL, "christian_name" character varying NOT NULL, "email" character varying NOT NULL, "password" character varying NOT NULL, "gender" "public"."student_gender_enum" NOT NULL, "phoneNumber" character varying(15) NOT NULL, "id_card_image_path" character varying NOT NULL, "barcode" character varying, "role" "public"."student_role_enum" NOT NULL DEFAULT 'student', "is_verified" boolean NOT NULL DEFAULT false, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "confessionFatherId" integer, CONSTRAINT "UQ_a56c051c91dbe1068ad683f536e" UNIQUE ("email"), CONSTRAINT "PK_3d8016e1cb58429474a3c041904" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "department" ("id" SERIAL NOT NULL, "department_name" character varying NOT NULL, CONSTRAINT "UQ_980e3e1f25ca867c47e38021bfc" UNIQUE ("department_name"), CONSTRAINT "PK_9a2213262c1593bffb581e382f5" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "language" ("id" SERIAL NOT NULL, "language_name" character varying NOT NULL, CONSTRAINT "PK_cc0a99e710eb3733f6fb42b1d4c" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "service_group" ("id" SERIAL NOT NULL, "service_name" character varying NOT NULL, "description" text NOT NULL, "presidentId" integer, "vicePresidentId" integer, "secretaryId" integer, CONSTRAINT "PK_3c836a1da7ea3fc6390a05cc573" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "service_member" ("user_id" integer NOT NULL, "serviceGroup" integer NOT NULL, "service_role" character varying NOT NULL, "studentId" integer, "groupId" integer, "subGroupId" integer, CONSTRAINT "PK_c96a305301b9f4a0d4523639ee3" PRIMARY KEY ("user_id", "serviceGroup"))`);
        ...
        await queryRunner.query(`CREATE TABLE "academic_info" ("id" SERIAL NOT NULL, "year" character varying, "dorm_block" character varying, "room_number" character varying, "userId" integer, "departmentId" integer, CONSTRAINT "REL_80c2ae2808bfe54d36a12aed49" UNIQUE ("userId"), CONSTRAINT "PK_f8f803c5c51f95882d24da87976" PRIMARY KEY ("id"))`);
...
//# sourceMappingURL=1757764929246-IntializingTable.js.map