...
export const getOverallAnalytics = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const attendanceStats: { status: "present" | "late" | "absent"; count: string }[] =
    ...
    const statsSummary: Record<"present" | "late" | "absent", number> = {
      present: 0,
      late: 0,
      absent: 0,
    };
...
// --------------------------------------
export const getCourseAttendanceStats = async (req: Request, res: Response): Promise<void> => {
  try {
