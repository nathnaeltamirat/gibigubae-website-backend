...
export const createSubGroup = async (
  ...
  try {
    ...
    const { serviceId, name, description, president, vice_president, secretary } =
      req.body;

    const parentGroup = await groupRepo.findOneBy({ id: serviceId });
    if (!parentGroup) {
      const error: CustomError = new Error("Parent service group not found");
      error.statusCode = 404;
      throw error;
    }
...
export const deleteSubGroup = async (
  ...
  try {
    ...
    if (user.role !== "super_admin") {
      const error: CustomError = new Error(
        "Forbidden: Only Super Admin can delete"
      );
      error.statusCode = 403;
      throw error;
    }
